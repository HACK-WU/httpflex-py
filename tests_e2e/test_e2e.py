...
启动本地 demo 服务器（tests_e2e/demo_server.py），用 httpflex 对其发起真实 HTTP 请求，
覆盖库的各种**用法**并验证其正确性：
...
from tests_e2e.demo_server import run_demo_server, reset_demo_state


# ========== 真实 Redis 连接配置（来自 .env 中的环境变量） ==========
# 地址/密码等敏感信息统一通过环境变量注入，见仓库根目录 .env（含 .env.example 模板）。
...
REDIS_PASSWORD = _redis_password or None  # 空字符串视为无密码
...
class TestErrorsUsage:
    ...
    def test_network_error(self):
        ...
        # 显式关闭代理，避免开发环境下的 HTTP 代理拦截该请求并返回 502 等网关错误，
        # 从而掩盖真正的"连接被拒绝"网络错误。
        client = client_for("http://127.0.0.1:1", "/get", proxies={"http": None, "https": None})
